...
export class EventService {
  ...
  private getUpComingEventsSummary(): EventsSummaryDto[] {
    return this.#eventSummary.filter(event => event.date >= new Date().toISOString()).sort((a, b) => {
      if (a.date > b.date) {
        return 1;
  ...
  private async getEvents(eventsSummaryDto: EventsSummaryDto[]) {
    ...
    for (const eventSummaryDto of eventsSummaryDto) {
      ...
      if (currentFileId != previousFileId) {
        ...
        currentFileContent = (await axios.get(backendRoutes.baseUrl + '/' + backendRoutes.event.replace("%d", currentFileId.toString()))).data;
      }
